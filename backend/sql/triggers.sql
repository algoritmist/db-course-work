 ...
 returns trigger as $$
        begin
                if 3=any(select НОМЕР_СТАТУСА from ЧЕЛОВЕК
                           ...
                           (select (ЧЛВК_ИД) from ВОИН where ИД=new.ПРЕДВОДИТЕЛЬ_ИД)) or 'ПОЛКОВОДЕЦ'=any(select НАЗВАНИЕ from ЧЕЛОВЕК
                           join СТАТУС on ЧЕЛОВЕК.СТАТУС_ИД=СТАТУС.ИД where ЧЕЛОВЕК.ИД=
...
$$ language plpgsql;
 ...
 create or replace trigger warrior_upd_com after update on ЧЕЛОВЕК for each row execute procedure predv_upd_status();create or replace function non_number()
returns trigger as $$
  begin
    if NEW.ИМЯ similar to '%[0-9]+%' THEN
      RAISE NOTICE 'НЕПРАВИЛЬНЫЙ ВВОД ИМЕНИ';
